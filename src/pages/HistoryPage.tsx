...
const HistoryPage: React.FC = () => {
  ...
  // 1. Load History (Guest Mode Compatible)
  useEffect(() => { 
    if (isLoaded) {
      // Use "guest" key if no user is logged in
  ...
  const getTimestamp = (item: ScanRecord) => item.timestamp;
  ...
  // Safe Probability Parsing (Preserves your original logic)
  const safeParseProbabilities = (item: any): Record<string, number> | null => {
    try {
      // Check if probabilities exist on the record (ScanRecord might allow extra fields)
  ...
  const stats = {
    ...
    normalScans: history.filter(i => normalizeDisease(i.prediction) === 'normal').length,
    abnormalScans: history.filter(i => normalizeDisease(i.prediction) !== 'normal').length,
  ...
  const downloadSinglePDF = (item: ScanRecord) => {
    // Map ScanRecord to the format generateReport expects
    const mockResult = {
      ...
      <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{duration:0.6}} className="space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="text-center md:text-left">
            ...
            <p className="text-gray-500">Track your eye health over time with detailed analysis history.</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card><CardContent className="p-6 text-center"><div className="text-2xl font-bold">{stats.totalScans}</div><div className="text-xs text-gray-500">Total Scans</div></CardContent></Card>
          <Card><CardContent className="p-6 text-center"><div className="text-2xl font-bold text-green-600">{stats.normalScans}</div><div className="text-xs text-gray-500">Normal</div></CardContent></Card>
          <Card><CardContent className="p-6 text-center"><div className="text-2xl font-bold text-red-600">{stats.abnormalScans}</div><div className="text-xs text-gray-500">Issues Found</div></CardContent></Card>
          <Card><CardContent className="p-6 text-center"><div className="text-2xl font-bold text-blue-600">{stats.avgConfidence}%</div><div className="text-xs text-gray-500">Avg. Confidence</div></CardContent></Card>
        </div>
                  ...
                  <Card className="hover:shadow-md transition-all">
                    <CardContent className="p-4 flex flex-col sm:flex-row items-start sm:items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {/* Optional Probability Grid (Restored from your original code) */}
                        {probabilities && (
                      ...
                      <div className="flex gap-2 self-end sm:self-center">
                        ...
                        <Button variant="outline" size="sm" onClick={() => downloadSinglePDF(item)} className="gap-2">

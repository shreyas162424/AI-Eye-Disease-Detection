    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl relative">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-8">
        ...
        <Card className={`shadow-lg border-2 ${isNormal ? "border-green-200 bg-green-50" : "border-yellow-200 bg-yellow-50"}`}>
          <CardHeader><div className="flex items-center gap-3">{isNormal ? <CheckCircle className="h-8 w-8 text-green-600" /> : <AlertCircle className="h-8 w-8 text-yellow-600" />}<div><CardTitle className="text-2xl">{t(result.predicted_disease) || friendlyLabel}</CardTitle><CardDescription>Confidence: {confidence}%</CardDescription></div></div></CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="space-y-4"><h3 className="font-semibold">Description</h3><p className="text-sm text-muted-foreground">{info.description}</p><Badge variant={isNormal ? "secondary" : "destructive"}>{info.severity}</Badge><p className="text-sm font-medium text-yellow-700">{info.urgency}</p></div>
              {(imageUrl && maskSrc) ? (
                <div className="space-y-4"><h3 className="font-semibold flex gap-2"><Info size={16} /> Lesion Detection</h3><CompareSlider original={imageUrl} overlay={maskSrc} /><Button variant="ghost" size="sm" onClick={() => setModalSrc(maskSrc)} className="w-full text-xs"><ZoomIn size={14} className="mr-1"/> View Fullscreen</Button></div>
              ) : imageUrl ? (<div className="rounded-lg overflow-hidden border"><img src={imageUrl} alt="Original" className="w-full h-64 object-cover" /></div>) : null}
            </div>
          </CardContent>
        </Card>
